    ...
    if (redirectToOkta) {
      ...
      if (oktaBaseUrl) {
        if (idTokenToUse) {
          ...
          console.log('⚠️ [Signout API] No ID token available - skipping Okta logout');
        }
      }
    }
  ...
  try {
    ...
    if (redirectToOkta && oktaBaseUrl) {
      if (idTokenToUse) {
        ...
        console.log('⚠️ [Signout API] No ID token available - skipping Okta logout, clearing local cookies only');
